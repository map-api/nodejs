...
import { collection, doc, setDoc,getFirestore,query,startAfter,getDocs,addDoc,where,deleteDoc, connectFirestoreEmulator ,limit, orderBy,updateDoc  } from "firebase/firestore"; 
...
export default class FirestoreAccessor {
  ...
  async jwtAuthorize(table, keyColumns, selectColumns, body) {
    ...
    let result = await conn.query(
      `SELECT ${_columns} FROM ${table} WHERE ${cond}`,_value);
    conn.close();
    conn.end();
  ...
  async select(collections, fieldList, condition, paging) {
    ...
    if(condition && condition.page)  delete condition.page;
      ...
      else
      {
        ...
        const querySnapshot = await getDocs(q).then((e)=>{})
        .catch((e)=>{
          console.error(e);
          return {
            affectedRows: 0,
  ...
  async update(collections, fieldList, valueList, condition, modelObject,queryOption){
    if(!fieldList || !valueList || (fieldList.length != valueList.length)){
      throw new InvalidSqlInsertExecuteException(
    ...
    if(querySnapshot.docs.length==0)
      {
        ...
        //새로Adddoc해주기!
        const ref=collection(db,collections);
        await addDoc(ref,data_set).then((e)=>{           
          const return_val={
            code:201,
            insertId: e.id,
            affectedRows:1,
        ...
        var val_=[];
        await updateDoc(doc(db,collections,last.id),data_set).then((e)=>{
        const result={
          code: code,
  ...
  async delete(table, condition){
    ...
    querySnapshot.forEach((docs)=>{    
      const ks=deleteDoc(doc(db,table, docs.id)).then((i)=>{}).catch((error)=>{  
        return  {
          code: 204,
