...
import {
  ...
  FileTransferHttpRequestHandler,
  JsonWebTokenHttpRequestHandler,
...
/* Rest Api */
if (
  baseConfigReader.getConfig()[API_TYPE.REST].use &&
  baseConfigReader.getConfig()[API_TYPE.REST].use === "yes"
) {
...
/* File Transfer */
if (
  baseConfigReader.getConfig()[API_TYPE.FILE_TRANSFER].use &&
  baseConfigReader.getConfig()[API_TYPE.FILE_TRANSFER].use === "yes"
) {
  ...
  const fileTransferHttpRequestHandler = new FileTransferHttpRequestHandler(
    app
  );
...
/* JWT */
if (
  baseConfigReader.getConfig()[API_TYPE.JWT].use &&
  baseConfigReader.getConfig()[API_TYPE.JWT].use === "yes"
) {
...
if (corsList.origin.length === 1) {
  if (
    corsList.origin[0] !== "*" &&
    !corsList.origin.includes(corsList.default)
  ) {
